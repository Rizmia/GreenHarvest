...
function Crop_Add() {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'crop_name' && value.length < 2 && value.length > 0) {
    ...
    // Fertilizer quantity validation
    if (name === 'Fertilizer_quantity' && value) {
      const num = Number(value);
      if (num < 0) {
        newErrors.Fertilizer_quantity = 'Fertilizer quantity cannot be negative';
      }
    }
    ...
    // Date validation
    if ((name === 'planting_date' || name === 'harvest_time') && inputs.planting_date && inputs.harvest_time) {
  ...
  const validateForm = () => {
    ...
    if (inputs.crop_name && inputs.crop_name.length < 2) {
      newErrors.crop_name = 'Crop name must be at least 2 characters';
    }
      ...
      {showPopup && (
        <div className="crop-add-popup-overlay">
          <div className="crop-add-popup">
            <h2>{missingFields.length > 0 ? 'Missing Fields' : 'Add New Crop Details'}</h2>
            ...
            <div className="crop-add-popup-buttons">
              ...
              <button onClick={cancelAdd} className="crop-add-cancel-btn">Back</button>
            </div>
          </div>
        </div>
      )}
